...
from yacut import constants, db
...
class URLMap(db.Model):
    ...
    def validate_data(data):
        ...
        if not data:
            raise URLValidationError(constants.ERROR_NO_BODY)
        elif "url" not in data:
            raise URLValidationError(constants.ERROR_URL_REQUIRED)
            ...
            raise URLValidationError(constants.ERROR_INVALID_CUSTOM_ID)
        elif URLMap.get_obj_by_short(data["custom_id"]) is not None:
            raise URLValidationError(constants.ERROR_CUSTOM_ID_EXISTS)
        return data
